...
contract MetaStablePool is StablePool, StableOracleMath, PoolPriceOracle {
    ...
    function _updateOracle(
        ...
        if (miscData.oracleEnabled() && block.number > lastChangeBlock) {
            (int256 logSpotPrice, int256 logBptPrice) = StableOracleMath
                ._calcLogPrices(
                    currentAmp,
                    balance0,
                    balance1,
     ...
     */
    function setPriceRateCacheDuration(IERC20 token, uint256 duration)
        external
        authenticate
    {
     ...
     */
    function _getPriceRateCacheValue(bytes32 cache)
        private
        pure
        returns (uint256)
    {
     ...
     */
    function _getPriceRateCacheDuration(bytes32 cache)
        private
        pure
        returns (uint256)
    {
    ...
    function _updateToken0PriceRateCache() private {
        _updateToken0PriceRateCache(
            _getPriceRateCacheDuration(_getPriceRateCache0())
        );
    }
    ...
    function _updateToken1PriceRateCache() private {
        _updateToken1PriceRateCache(
            _getPriceRateCacheDuration(_getPriceRateCache1())
        );
    }
    ...
    function _isToken0WithRateProvider(IERC20 token)
        internal
        view
        returns (bool)
    {
        return
            _isToken0(token) &&
            _getRateProvider0() != IRateProvider(address(0));
    }

    function _isToken1WithRateProvider(IERC20 token)
        internal
        view
        returns (bool)
    {
        return
            _isToken1(token) &&
            _getRateProvider1() != IRateProvider(address(0));
    }
