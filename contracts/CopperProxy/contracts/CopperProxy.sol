...
// import enumerable set from openzeppelin
...
contract CopperProxy is Ownable {
    ...
    function createAuction(PoolConfig memory poolConfig)
        external
        returns (address)
    {
    ...
    function exitPool(
        ...
        if (fundTokenBalance > poolData.fundTokenInputAmount) {
            uint256 feeAmount = ((fundTokenBalance -
                poolData.fundTokenInputAmount) * _feeBPS) / 10_000;
            TransferHelper.safeTransfer(fundToken, _feeRecipient, feeAmount);
            emit TransferedFee(pool, fundToken, _feeRecipient, feeAmount);
        }

        // 5. Transfer to user
        uint256 firstTokenBalance = IERC20(poolTokens[0]).balanceOf(
            address(this)
        );
        ...
        uint256 secondTokenBalance = IERC20(poolTokens[1]).balanceOf(
            address(this)
        );
        TransferHelper.safeTransfer(
            poolTokens[1],
