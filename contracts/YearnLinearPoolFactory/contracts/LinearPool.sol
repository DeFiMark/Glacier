...
abstract contract LinearPool is BasePool, IGeneralPool, IRateProvider {
        ...
        )
    {
        ...
        (
            ...
            uint256 bptIndex
        ) = _getSortedTokenIndexes(mainToken, wrappedToken, this);
        _bptIndex = bptIndex;
        ...
        returns (uint256)
    {
        ...
        _require(
            indexIn < _TOTAL_TOKENS && indexOut < _TOTAL_TOKENS,
    ...
    function _swapGivenWrappedIn(
        ...
        _require(
            request.tokenOut == _mainToken || request.tokenOut == this,
            Errors.INVALID_TOKEN
        );
        return
            request.tokenOut == this
    ...
    function _swapGivenMainOut(
        ...
        _require(
            request.tokenIn == _wrappedToken || request.tokenIn == this,
            Errors.INVALID_TOKEN
        );
        return
    ...
    function _swapGivenWrappedOut(
        ...
        _require(
            request.tokenIn == _mainToken || request.tokenIn == this,
            Errors.INVALID_TOKEN
        );
        return
            request.tokenIn == this
        ...
        )
    {
        ...
        if (
            kind !=
            LinearPoolUserData.ExitKind.EMERGENCY_EXACT_BPT_IN_FOR_TOKENS_OUT
        ) {
            _revert(Errors.UNHANDLED_BY_LINEAR_POOL);
        } else {
            _ensurePaused();
            ...
            (bptAmountIn, amountsOut) = _emergencyProportionalExit(
                balances,
                userData
            );
    ...
    function _emergencyProportionalExit(
        uint256[] memory balances,
        bytes memory userData
        ...
        returns (uint256[] memory)
    {
        ...
        scalingFactors[_wrappedIndex] = _scalingFactorWrappedToken.mulDown(
    ...
    function _setTargets(
        ...
        _require(
            ...
            Errors.LOWER_GREATER_THAN_UPPER_TARGET
        );
        _require(
            upperTarget <= _MAX_UPPER_TARGET,
            Errors.UPPER_TARGET_TOO_HIGH
        );
    ...
    function setSwapFeePercentage(uint256 swapFeePercentage) public override {
        ...
        (uint256 lowerTarget, uint256 upperTarget) = getTargets();
        _require(
            _isMainBalanceWithinTargets(lowerTarget, upperTarget),
            Errors.OUT_OF_TARGET_RANGE
        );
    ...
    function _isMainBalanceWithinTargets(
        uint256 lowerTarget,
        uint256 upperTarget
    ) private view returns (bool) {
        bytes32 poolId = getPoolId();
        (, uint256[] memory balances, ) = getVault().getPoolTokens(poolId);
        uint256 mainTokenBalance = _upscale(
            balances[_mainIndex],
            _scalingFactor(_mainToken)
        );

        return
            mainTokenBalance >= lowerTarget && mainTokenBalance <= upperTarget;
    }
    ...
    function _getVirtualSupply(uint256 bptBalance)
        internal
        view
        returns (uint256)
    {
     ...
     * exit process.
     */
    function _getApproximateVirtualSupply(uint256 bptBalance)
        internal
        pure
        returns (uint256)
    {
