...
import "../abstract/TemporarilyPausable.sol";
import "../libraries/ERC20.sol";

import "./WeightedMath.sol";
import "./WeightedOracleMath.sol";
import "../libraries/WeightedPool2TokensMiscData.sol";
import "../libraries/WeightedPoolUserDataHelpers.sol";
import "../abstract/BalancerPoolToken.sol";
import "../abstract/BasePoolAuthorization.sol";
    ...
    WeightedOracleMath
{
    ...
    modifier onlyVault(bytes32 poolId) {
        Errors._require(msg.sender == address(getVault()), Errors.CALLER_NOT_VAULT);
        Errors._require(poolId == getPoolId(), Errors.INVALID_POOL_ID);
        _;
    }
        ...
        TemporarilyPausable(params.pauseWindowDuration, params.bufferPeriodDuration)
    {
        ...
        Errors._require(params.normalizedWeight0 >= _MIN_WEIGHT, Errors.MIN_WEIGHT);
        Errors._require(params.normalizedWeight1 >= _MIN_WEIGHT, Errors.MIN_WEIGHT);

        // Ensure that the normalized weights sum to ONE
        uint256 normalizedSum = params.normalizedWeight0.add(params.normalizedWeight1);
        Errors._require(normalizedSum == FixedPoint.ONE, Errors.NORMALIZED_WEIGHT_INVARIANT);
    ...
    function _setSwapFeePercentage(uint256 swapFeePercentage) private {
        Errors._require(swapFeePercentage >= _MIN_SWAP_FEE_PERCENTAGE, Errors.MIN_SWAP_FEE_PERCENTAGE);
        Errors._require(swapFeePercentage <= _MAX_SWAP_FEE_PERCENTAGE, Errors.MAX_SWAP_FEE_PERCENTAGE);
        ...
        returns (uint256[] memory amountsIn, uint256[] memory dueProtocolFeeAmounts)
    {
        ...
        if (totalSupply() == 0) {
            ...
            Errors._require(bptAmountOut >= _MINIMUM_BPT, Errors.MINIMUM_BPT);
    ...
    function _onInitializePool(
        ...
        Errors._require(kind == WeightedPool.JoinKind.INIT, Errors.UNINITIALIZED);
    ...
    function _joinTokenInForExactBPTOut(
        ...
        Errors._require(tokenIndex < 2, Errors.OUT_OF_BOUNDS);
        ...
        returns (uint256[] memory results)
    {
        ...
        for (uint256 i = 0; i < queries.length; ++i) {
            query = queries[i];
            Errors._require(query.secs != 0, Errors.ORACLE_BAD_SECS);
