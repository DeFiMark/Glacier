...
contract PoolPriceOracle is IWeightedPoolPriceOracle {
    ...
    {
        Errors._require(index < Buffer.SIZE, Errors.ORACLE_INVALID_INDEX);
    ...
    function _getInstantValue(IPriceOracle.Variable variable, uint256 index) internal view returns (int256) {
        bytes32 sample = _getSample(index);
        Errors._require(sample.timestamp() > 0, Errors.ORACLE_NOT_INITIALIZED);
    ...
    function _getPastAccumulator(
        ...
        Errors._require(block.timestamp >= ago, Errors.ORACLE_INVALID_SECONDS_QUERY);
        ...
        Errors._require(latestTimestamp > 0, Errors.ORACLE_NOT_INITIALIZED);
        ...
        if (latestTimestamp <= lookUpTime) {
            ...
            uint256 oldestIndex = latestIndex.next();
            {
                ...
                Errors._require(oldestTimestamp > 0, Errors.ORACLE_NOT_INITIALIZED);
                // The only remaining condition to check is for the look up time to be between the oldest and latest
                // timestamps.
                Errors._require(oldestTimestamp <= lookUpTime, Errors.ORACLE_QUERY_TOO_OLD);
            }
