...
import "../libraries/InputHelpers.sol";
import "../libraries/WordCodec.sol";

import "../abstract/BaseStableGeneralPool.sol";
import "../abstract/BaseMinimalSwapInfoStablePool.sol";

import "../abstract/StableMath.sol";
import "../libraries/StablePoolUserDataHelpers.sol";

contract StablePool is BaseStableGeneralPool, BaseMinimalSwapInfoStablePool, StableMath {
    ...
    constructor(
        ...
        BasePool(
            vault,
            // Because we're inheriting from both BaseGeneralPool and BaseMinimalSwapInfoStablePool we can choose any
    ...
    {
        Errors._require(tokens.length <= _MAX_STABLE_TOKENS, Errors.MAX_STABLE_TOKENS);

        Errors._require(amplificationParameter >= _MIN_AMP, Errors.MIN_AMP);
        Errors._require(amplificationParameter <= _MAX_AMP, Errors.MAX_AMP);
    ...
    // Swap - Two Token Pool specialization (from BaseMinimalSwapInfoStablePool)
    ...
    function _onSwapGivenIn(
        ...
        Errors._require(_getTotalTokens() == 2, Errors.NOT_TWO_TOKENS);
    ...
    ) internal virtual override(BaseStablePool) whenNotPaused returns (uint256, uint256[] memory) {
        // It would be strange for the Pool to be paused before it is initialized, but for consistency we prevent
        // initialization in this case.

        StablePool.JoinKind kind = userData.joinKind();
        Errors._require(kind == StablePool.JoinKind.INIT, Errors.UNINITIALIZED);
    ...
    function _doJoin(
        ...
        if (kind == JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT) {
            ...
            Errors._revert(Errors.UNHANDLED_JOIN_KIND);
        }
    }
    ...
    function _joinExactTokensInForBPTOut(
        ...
        Errors._require(bptAmountOut >= minBPTAmountOut, Errors.BPT_OUT_MIN_AMOUNT);
        ...
        returns (uint256, uint256[] memory)
    {
        ...
        Errors._require(tokenIndex < _getTotalTokens(), Errors.OUT_OF_BOUNDS);
    ...
    function _exitBPTInForExactTokensOut(
        ...
        Errors._require(bptAmountIn <= maxBPTAmountIn, Errors.BPT_IN_MAX_AMOUNT);
    ...
    function startAmplificationParameterUpdate(uint256 rawEndValue, uint256 endTime) external authenticate {
        Errors._require(rawEndValue >= _MIN_AMP, Errors.MIN_AMP);
        Errors._require(rawEndValue <= _MAX_AMP, Errors.MAX_AMP);

        uint256 duration = Math.sub(endTime, block.timestamp);
        Errors._require(duration >= _MIN_UPDATE_TIME, Errors.AMP_END_TIME_TOO_CLOSE);

        (uint256 currentValue, bool isUpdating) = _getAmplificationParameter();
        Errors._require(!isUpdating, Errors.AMP_ONGOING_UPDATE);
        ...
        Errors._require(dailyRate <= _MAX_AMP_UPDATE_DAILY_RATE, Errors.AMP_RATE_TOO_HIGH);
    ...
    function stopAmplificationParameterUpdate() external authenticate {
        (uint256 currentValue, bool isUpdating) = _getAmplificationParameter();
        Errors._require(isUpdating, Errors.AMP_NO_ONGOING_UPDATE);

        _setAmplificationData(currentValue);
    }

    function _isOwnerOnlyAction(bytes32 actionId) internal view virtual override(BaseStablePool) returns (bool) {
        return
