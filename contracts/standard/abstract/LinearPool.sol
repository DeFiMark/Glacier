...
import "../libraries/PriceRateCache.sol";

import "../interfaces/IGeneralPool.sol";

import "../libraries/LinearMath.sol";
import "../libraries/LinearPoolUserData.sol";
...
abstract contract LinearPool is BasePool, IGeneralPool, IRateProvider {
    ...
    constructor(
        ...
        BasePool(
            ...
            name,
            symbol,
            ERC20Helper._sortTokens(mainToken, wrappedToken, this),
        ...
        )
    {
        ...
        // Set token indexes
        (uint256 mainIndex, uint256 wrappedIndex, uint256 bptIndex) = ERC20Helper._getSortedTokenIndexes(
            mainToken,
            wrappedToken,
            this
        );
    ...
    function initialize() external {
        ...
        IVault.JoinPoolRequest memory request = IVault.JoinPoolRequest({
            assets: ERC20Helper._asIAsset(tokens),
    ...
    function onSwap(
        ...
        Errors._require(indexIn < _TOTAL_TOKENS && indexOut < _TOTAL_TOKENS, Errors.OUT_OF_BOUNDS);
    ...
    function _onSwapGivenIn(
        ...
        if (request.tokenIn == this) {
            ...
            Errors._revert(Errors.INVALID_TOKEN);
        }
    }
    ...
    function _swapGivenBptIn(
        ...
        Errors._require(request.tokenOut == _mainToken || request.tokenOut == _wrappedToken, Errors.INVALID_TOKEN);
        return
    ...
    function _swapGivenMainIn(
        ...
        Errors._require(request.tokenOut == _wrappedToken || request.tokenOut == this, Errors.INVALID_TOKEN);
        return
    ...
    function _swapGivenWrappedIn(
        ...
        Errors._require(request.tokenOut == _mainToken || request.tokenOut == this, Errors.INVALID_TOKEN);
        return
            request.tokenOut == this
    ...
    function _swapGivenBptOut(
        ...
        Errors._require(request.tokenIn == _mainToken || request.tokenIn == _wrappedToken, Errors.INVALID_TOKEN);
        return
    ...
    function _swapGivenMainOut(
        ...
        Errors._require(request.tokenIn == _wrappedToken || request.tokenIn == this, Errors.INVALID_TOKEN);
        return
    ...
    function _swapGivenWrappedOut(
        ...
        Errors._require(request.tokenIn == _mainToken || request.tokenIn == this, Errors.INVALID_TOKEN);
        return
            request.tokenIn == this
    ...
    function _onInitializePool(
        ...
        Errors._require(sender == address(this), Errors.INVALID_INITIALIZATION);
        Errors._require(recipient == address(this), Errors.INVALID_INITIALIZATION);
    ...
    {
        Errors._revert(Errors.UNHANDLED_BY_LINEAR_POOL);
    }
    ...
    function _setTargets(
        ...
        Errors._require(lowerTarget <= upperTarget, Errors.LOWER_GREATER_THAN_UPPER_TARGET);
        Errors._require(upperTarget <= _MAX_UPPER_TARGET, Errors.UPPER_TARGET_TOO_HIGH);
    ...
    function setTargets(uint256 newLowerTarget, uint256 newUpperTarget) external authenticate {
        ...
        Errors._require(_isMainBalanceWithinTargets(currentLowerTarget, currentUpperTarget), Errors.OUT_OF_TARGET_RANGE);
        Errors._require(_isMainBalanceWithinTargets(newLowerTarget, newUpperTarget), Errors.OUT_OF_NEW_TARGET_RANGE);
    ...
    function setSwapFeePercentage(uint256 swapFeePercentage) public override {
        ...
        (uint256 lowerTarget, uint256 upperTarget) = getTargets();
        Errors._require(_isMainBalanceWithinTargets(lowerTarget, upperTarget), Errors.OUT_OF_TARGET_RANGE);
