...
abstract contract UserBalance is ReentrancyGuard, AssetTransfersHandler, VaultAuthorization {
        ...
        for (uint256 i = 0; i < ops.length; i++) {
            ...
            if (kind == UserBalanceOpKind.WITHDRAW_INTERNAL) {
                ...
                if (kind == UserBalanceOpKind.DEPOSIT_INTERNAL) {
                    ...
                    Errors._require(!_isETH(asset), Errors.CANNOT_USE_ETH_SENTINEL);
    ...
    function _decreaseInternalBalance(
        ...
        uint256 currentBalance = _getInternalBalance(account, token);
        Errors._require(allowPartial || (currentBalance >= amount), Errors.INSUFFICIENT_INTERNAL_BALANCE);
        ...
        )
    {
        ...
        if (sender != msg.sender) {
            ...
            Errors._require(_hasApprovedRelayer(sender, msg.sender), Errors.USER_DOESNT_ALLOW_RELAYER);
        }
