...
abstract contract PoolBalances is Fees, ReentrancyGuard, PoolTokens, UserBalance {
    ...
    function _processJoinPoolTransfers(
        ...
        for (uint256 i = 0; i < change.assets.length; ++i) {
            ...
            Errors._require(amountIn <= change.limits[i], Errors.JOIN_ABOVE_MAX);
    ...
    function _processExitPoolTransfers(
        ...
        for (uint256 i = 0; i < change.assets.length; ++i) {
            ...
            Errors._require(amountOut >= change.limits[i], Errors.EXIT_BELOW_MIN);
        ...
        returns (bytes32[] memory)
    {
        ...
        Errors._require(actualTokens.length > 0, Errors.POOL_NO_TOKENS);

        for (uint256 i = 0; i < actualTokens.length; ++i) {
            Errors._require(actualTokens[i] == expectedTokens[i], Errors.TOKENS_MISMATCH);
        }
