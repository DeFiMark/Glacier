...
import "../libraries/BalanceAllocation.sol";
...
abstract contract Swaps is ReentrancyGuard, PoolBalances {
        ...
        returns (uint256 amountCalculated)
    {
        ...
        Errors._require(block.timestamp <= deadline, Errors.SWAP_DEADLINE);

        // This revert reason is for consistency with `batchSwap`: an equivalent `swap` performed using that function
        // would result in this error.
        Errors._require(singleSwap.amount > 0, Errors.UNKNOWN_AMOUNT_IN_FIRST_SWAP);

        IERC20 tokenIn = _translateToIERC20(singleSwap.assetIn);
        IERC20 tokenOut = _translateToIERC20(singleSwap.assetOut);
        Errors._require(tokenIn != tokenOut, Errors.CANNOT_SWAP_SAME_TOKEN);
        ...
        Errors._require(singleSwap.kind == SwapKind.GIVEN_IN ? amountOut >= limit : amountIn <= limit, Errors.SWAP_LIMIT);
        ...
        returns (int256[] memory assetDeltas)
    {
        ...
        for (uint256 i = 0; i < assets.length; ++i) {
            ...
            Errors._require(delta <= limits[i], Errors.SWAP_LIMIT);
    ...
    function _swapWithPools(
        ...
        for (uint256 i = 0; i < swaps.length; ++i) {
            ...
            Errors._require(withinBounds, Errors.OUT_OF_BOUNDS);
            ...
            if (batchSwapStep.amount == 0) {
                ...
                Errors._require(i > 0, Errors.UNKNOWN_AMOUNT_IN_FIRST_SWAP);
                bool usingPreviousToken = previousTokenCalculated == _tokenGiven(kind, tokenIn, tokenOut);
                Errors._require(usingPreviousToken, Errors.MALCONSTRUCTED_MULTIHOP_SWAP);
        ...
        returns (uint256 amountCalculated)
    {
        ...
        if (indexIn == 0 || indexOut == 0) {
            ...
            Errors._revert(Errors.TOKEN_NOT_REGISTERED);
        }
