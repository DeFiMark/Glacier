...
abstract contract PoolRegistry is ReentrancyGuard, VaultAuthorization {
    ...
    function _ensureRegisteredPool(bytes32 poolId) internal view {
        Errors._require(_isPoolRegistered[poolId], Errors.INVALID_POOL_ID);
    }
    ...
    function _ensurePoolIsSender(bytes32 poolId) private view {
        _ensureRegisteredPool(poolId);
        Errors._require(msg.sender == _getPoolAddress(poolId), Errors.CALLER_NOT_POOL);
    }
        ...
        returns (bytes32)
    {
        ...
        Errors._require(!_isPoolRegistered[poolId], Errors.INVALID_POOL_ID); // Should never happen as Pool IDs are unique.
    ...
    function _getPoolSpecialization(bytes32 poolId) internal pure returns (PoolSpecialization specialization) {
        ...
        // values 0, 1 and 2.
        Errors._require(value < 3, Errors.INVALID_POOL_ID);
