...
abstract contract AssetTransfersHandler is AssetHelpers {
    ...
    function _receiveAsset(
        ...
        if (_isETH(asset)) {
            Errors._require(!fromInternalBalance, Errors.INVALID_ETH_INTERNAL_BALANCE);
            ...
            Errors._require(address(this).balance >= amount, Errors.INSUFFICIENT_ETH);
    ...
    function _sendAsset(
        ...
        if (_isETH(asset)) {
            ...
            Errors._require(!toInternalBalance, Errors.INVALID_ETH_INTERNAL_BALANCE);
    ...
    function _handleRemainingEth(uint256 amountUsed) internal {
        Errors._require(msg.value >= amountUsed, Errors.INSUFFICIENT_ETH);
    ...
    receive() external payable {
        Errors._require(msg.sender == address(_WETH()), Errors.ETH_TRANSFER);
    }
